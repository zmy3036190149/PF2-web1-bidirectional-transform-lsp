...
public class ProblemEditor {
	...
	/* xtext ->xmi(已有接口) */
	public static File performSave1(String filename) {
		...
		File xmlFile = new File(filename.replace(".pf", "pf.xml"));
		URI xmiURI = URI.createURI(xmlFile.getPath());
		Resource xmiResource = new XMIResourceFactoryImpl().createResource(xmiURI); // 创建特定的资源
		xmiResource.getContents().add(xtextResource.getContents().get(0));
			...
			for (Object object : linkElementList) {
				...
				for (Object obj : pheElementList) {
					...
					for (Phenomenon phe : pheList) {
						if (phe.getName() != null && phe.getName().equals(pheName) && phe.getType().equals(pheType)) {
							exit = true;
							break;
						}
					}
				...
				if (type == null) {
					...
					for (Phenomenon phe : phenomenonList) {
						...
						for (Requirement req : requirementList) {
							if (req.getName().equals(from) || req.getName().equals(to)) {
...
//					 node.add(description);
			...
			for (Object object : linkElementList) {
				...
				for (Object obj : pheElementList) {
					...
					boolean exit = false;
					for (Phenomenon phe : pheList) {
						if (phe.getName().equals(pheName) && phe.getType().equals(pheType)) {
							exit = true;
							break;
						}
					}
				...
				if (type == null) {
					...
					for (int i = 0; i < interfaceNum - 1; i++) {
						if (from.contentEquals(inteList.get(i).getTo()) && to.contentEquals(inteList.get(i).getFrom())
					...
					for (int i = 0; i < referenceNum - 1; i++) {
						if (from.contentEquals(refList.get(i).getTo()) && to.contentEquals(refList.get(i).getFrom())
...
//							 System.out.println(to);
					...
					for (int i = 0; i < constraintNum - 1; i++) {
						if (from.contentEquals(conList.get(i).getTo()) && to.contentEquals(conList.get(i).getFrom())
...
//			 System.out.println("---------------------");
	...
	/*
	 * 遍历pf文件 节点更新方式： 若简称相同，名称不同，则修改名称 若名称相同，简称不同，则修改名称 若简称、名称都不相同，则删除该节点 边更新方式
	 * 若存在，则跳过 若不存在，则删除
	 */
...
//		 fileOutputStream.close();
	...
	public static String changeReference(String from, String to, String type, String[] pheList, String name,
			Project project) {
		String line = "";
		// reference
		List<Reference> ref = project.getProblemDiagram().getReferenceList();
	...
	public static String changeConstraint(String from, String to, String type, String[] pheList, String name,
			Project project) {
		// constraint
		String line = "";
		List<Constraint> con = project.getProblemDiagram().getConstraintList();
...
//		 System.out.println("getShortname---------"+name+"-----");
	...
	public static String changeProblemDomain(String shortname, String type, String name, Project project) {
		...
		int pdlen = project.getContextDiagram().getProblemDomainList().size();
		for (int i = 0; i < pdlen; i++) {
			if (name.contentEquals(pd.get(i).getName()) || shortname.contentEquals(pd.get(i).getShortname())) {
	...
	/*
	 * 遍历project 节点更新方式 若存在，则跳过 若不存在，则添加到最后 边更新方式 若存在，则跳过 若不存在，则添加到最后
	 * 
	 */
...
//		 System.out.println("----------------------------------------------");
	...
	public static String searchInterface(String res, Interface intf, Boolean forward) throws IOException {
		...
		for (String line : lines) {
			...
			if (link_name.contentEquals(pfLink.getName()) && forward && link_from.contentEquals(pfLink.getFrom())) {
				if (flag) {
...
//				 break;
//			}
			else if (link_name.contentEquals(pfLink.getName()) && (!forward) && link_from.contentEquals(pfLink.getTo())
...
//				 System.out.println(line+" 不存在，加到最后");
	...
	public static String getInterface(Interface intf, Boolean forward) {
		...
		for (Phenomenon phe : pheList) {
			if (phe.getFrom().contentEquals(from) && phe.getTo().contentEquals(to)) {
				pheList1 += phe.getType() + " " + phe.getName() + ",";
			} else if (phe.getFrom().contentEquals(to) && phe.getTo().contentEquals(from)) {
				flag = true;// 另一个方向没有现象
			}
		}
		pheList1 = pheList1.substring(0, pheList1.length() - 1);
...
//				 System.out.println("to:" + intej.getInterface_to());
				if (intei.getFrom().equals(intej.getTo()) && intei.getTo().equals(intej.getFrom())) {
				...
				if (referencei.getFrom().equals(referencej.getTo())
					...
					if (referencej.getDescription().split(":").length == 1) {
						...
						if (referencei.getDescription().split(":").length == 1) {
							...
							description = description + referencej.getDescription().split(":")[1] + ","
									+ referencej.getDescription().split(":")[1];
						}
					}
				...
				if (constrainti.getFrom().equals(constraintj.getTo())
					...
					if (constraintj.getDescription().split(":").length == 1) {
						...
						if (constrainti.getDescription().split(":").length == 1) {
							...
							description = description + constrainti.getDescription().split(":")[1] + ","
									+ constraintj.getDescription().split(":")[1];
						}
					}
